...
public class MySQLConnection implements DBConnection {
	...
	public String getFullname(String userId) {
		/*
		 * if (conn == null) { return null; } StringBuilder fullName = new
		 * StringBuilder(); try { String sql =
		 * "SELECT first_name, last_name from users WHERE user_id = ? ";
		 * PreparedStatement statement = conn.prepareStatement(sql);
		 * statement.setString(1, userId); ResultSet rs = statement.executeQuery(); if
		 * (rs.next()) { fullName.append(rs.getString("first_name"));
		 * fullName.append(" "); fullName.append(rs.getString("last_name")); } } catch
	...
	public boolean verifyLogin(String userId, String password) {
		/*
		 * if (conn == null) { return false; } // String check = ""; try { String sql =
		 * "SELECT user_id from users WHERE user_id = ? AND password = ?";
		 * PreparedStatement statement = conn.prepareStatement(sql);
		 * statement.setString(1, userId); statement.setString(2, password); ResultSet
		 * rs = statement.executeQuery(); if (rs.next()) { // check =
		 * rs.getString("password"); return true; } } catch (Exception e) {
		 * System.out.println(e.getMessage()); } // return check == password; return
		 * false;
		 */
		if (conn == null) {
			return false;
